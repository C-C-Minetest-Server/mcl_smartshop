...
local S = smartshop.translator

minetest.register_craft({
	output = "mcl_smartshop:shop",
	recipe = {
		{"mcl_chests:chest", "mcl_chests:chest", "mcl_chests:chest"},
		{"mcl_signs:wall_sign", "mcl_chests:chest", "mcl_signs:wall_sign"},
		{"mcl_signs:wall_sign", "mcl_torches:torch", "mcl_signs:wall_sign"},
	}
})

minetest.register_craft({
	output = "mcl_smartshop:wifistorage",
	recipe = {
		{"mcl_core:gold_ingot", "mcl_chests:chest", "mcl_core:gold_ingot"},
		{"mcl_core:gold_ingot", "mcl_chests:ender_chest", "mcl_core:gold_ingot"},
		{"mcl_core:iron_ingot", "mcl_core:diamond", "mcl_core:iron_ingot"},
	}
})

-- TODO: Replace this with minetest.pos_to_string whenever possible
...
smartshop.get_offer = function(pos)
	...
	if minetest.get_node(pos).name ~= "mcl_smartshop:shop" then return end
...
smartshop.receive_fields = function(player,pressed)
		...
		elseif pressed.tsend then
			...
			minetest.after(30, function(pname,player)
				if smartshop.add_storage[pname] then
					if player:is_player() then -- Avoid player offline ERROR
...
minetest.register_entity("mcl_smartshop:item",{
	hp_max = 1,
	visual = "wielditem",
...
smartshop.showform = function(pos,player,re)
	...
	if minetest.check_player_privs(uname, {server = true}) then owner = true end
	if re then owner = false end
	smartshop.user[uname] = pos
	if owner then
		if meta:get_int("type") == 0 and not(minetest.is_creative_enabled(uname)
		...
		gui = gui
		.."list[nodemeta:" .. spos .. ";main;0,2;9,4;]"
		..mcl_formspec.get_itemslot_bg(0,2,9,4)
		.."label[0,6.0;"..minetest.formspec_escape(minetest.colorize("#313131", S("Inventory"))).."]"
		.."list[current_player;main;0,6.5;9,3;9]"
		..mcl_formspec.get_itemslot_bg(0,6.5,9,3)
		.."list[current_player;main;0,9.74;9,1;]"
		..mcl_formspec.get_itemslot_bg(0,9.74,9,1)
		.."listring[nodemeta:" .. spos .. ";main]"
		.."listring[current_player;main]"
	else
		gui=""
		.."size[9,9]"
		.."label[0,3.0;"..minetest.formspec_escape(minetest.colorize("#313131", S("Inventory"))).."]"
		.."list[current_player;main;0,3.5;9,3;9]"
		..mcl_formspec.get_itemslot_bg(0,3.5,9,3)
		.."list[current_player;main;0,6.74;9,1;]"
		..mcl_formspec.get_itemslot_bg(0,6.74,9,1)
		.."label[0,0.2;Item:]"
		.."label[0,1.2;Price:]"
...
minetest.register_node("mcl_smartshop:shop", {
	description = "Smartshop",
	tiles = {"default_gold_block.png^[colorize:#ffffff77^mcl_core_glass_black.png"},
	groups = {building_block=1, material_wood=1, container=1, flammable=-1, axey=1, tubedevice = 1, tubedevice_receiver = 1,mesecon=2},
	drawtype="nodebox",
...
smartshop.showform2 = function(pos,player)
	local meta = minetest.get_meta(pos)
	local uname = player:get_player_name()
	if meta:get_string("owner") ~= uname
		and not minetest.check_player_privs(uname, {protection_bypass = true})
		and not minetest.check_player_privs(uname, {server = true})
		then return end
	local inv = meta:get_inventory()
	local spos = pos.x .. "," .. pos.y .. "," .. pos.z
	...
	.."label[3,5.0;"..minetest.formspec_escape(minetest.colorize("#313131", S("Inventory"))).."]"
	.."list[current_player;main;3,5.5;9,3;9]"
	..mcl_formspec.get_itemslot_bg(3,5.5,9,3)
	.."list[current_player;main;3,8.74;9,1;]"
	..mcl_formspec.get_itemslot_bg(3,8.74,9,1)
	..mcl_formspec.get_itemslot_bg(0,0,12,5)
...
minetest.register_node("mcl_smartshop:wifistorage", {
	description = "Wifi storage",
	tiles = {"mcl_core_emerald_block.png^[colorize:#ffffffaa^mcl_core_glass_black.png"},
	groups = {building_block=1, container=2, material_wood=1, flammable=-1, axey=1, tubedevice_receiver = 1, mesecon=2},
	paramtype = "light",
	sunlight_propagates = true,
	light_source = 10,
	on_timer = function (pos, elapsed)
...
allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		if stack:get_wear() == 0
			...
			or minetest.check_player_privs(player:get_player_name(), {server=true}))
		then
		return stack:get_count()
		end
		return 0
...
allow_metadata_inventory_take = function(pos, listname, index, stack, player)
		if minetest.get_meta(pos):get_string("owner") == player:get_player_name()
			or minetest.check_player_privs(player:get_player_name(), {protection_bypass=true})
			or minetest.check_player_privs(player:get_player_name(), {server=true})
		then
		return stack:get_count()
...
can_dig = function(pos, player)
		...
		if (
				meta:get_string("owner") == p
				or minetest.check_player_privs(p, {protection_bypass = true})
				or minetest.check_player_privs(p, {server = true})
			)
			and inv:is_empty("main") or meta:get_string("owner") == "" then
			return true
		end
	end,
})
